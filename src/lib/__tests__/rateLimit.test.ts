...
describe('rateLimit', () => {
  ...
  describe('requests under the limit', () => {
    it('allows the first request', async () => {
      const result = await rateLimit('ip:1.2.3.4', 5, 60_000)
    ...
    it('allows requests up to the limit', async () => {
      for (let i = 1; i <= 5; i++) {
        const result = await rateLimit('ip:test-under', 5, 60_000)
        expect(result.ok).toBe(true)
      }
    })
  })
  ...
  describe('requests over the limit', () => {
    it('blocks the request immediately after the limit is reached', async () => {
      ...
      const limit = 3
      for (let i = 0; i < limit; i++) await rateLimit(key, limit, 60_000)
      const blocked = await rateLimit(key, limit, 60_000)
      expect(blocked.ok).toBe(false)
    })

    it('continues to block subsequent requests in the same window', async () => {
      const key = 'ip:keep-blocking'
      const limit = 2
      await rateLimit(key, limit, 60_000)
      await rateLimit(key, limit, 60_000)
      // Both next calls should be blocked
      expect((await rateLimit(key, limit, 60_000)).ok).toBe(false)
      expect((await rateLimit(key, limit, 60_000)).ok).toBe(false)
    })
  })
  ...
  describe('retryAfter', () => {
    it('is 0 when the request is allowed', async () => {
      expect((await rateLimit('ip:retry-0', 10, 60_000)).retryAfter).toBe(0)
    })

    it('is a positive integer in seconds when blocked', async () => {
      const key = 'ip:retry-seconds'
      const windowMs = 30_000 // 30-second window
      await rateLimit(key, 1, windowMs)       // consume the only slot
      const { retryAfter } = await rateLimit(key, 1, windowMs)
      // retryAfter must be between 1 and 30 (ceiling of remaining ms / 1000)
      expect(retryAfter).toBeGreaterThanOrEqual(1)
      expect(retryAfter).toBeLessThanOrEqual(30)
    })

    it('returns the ceiling of remaining window time', async () => {
      ...
      await rateLimit(key, 1, windowMs) // consumes the slot; resetAt = now + 5000
  ...
  describe('independent counters', () => {
    it('different keys do not share counters', async () => {
      await rateLimit('keyA', 1, 60_000) // exhaust keyA
      const resultB = await rateLimit('keyB', 1, 60_000) // keyB still fresh
    ...
    it('exhausting one IP does not affect another IP', async () => {
      const limit = 3
      const win = 60_000
      for (let i = 0; i < limit; i++) await rateLimit('ip:192.168.1.1', limit, win)
      expect((await rateLimit('ip:192.168.1.1', limit, win)).ok).toBe(false)
      expect((await rateLimit('ip:10.0.0.1',    limit, win)).ok).toBe(true)
    })
  })
  ...
  describe('window expiry', () => {
    it('allows requests again after the window expires', async () => {
      ...
      await rateLimit(key, limit, windowMs) // consume the slot
      expect((await rateLimit(key, limit, windowMs)).ok).toBe(false) // blocked
    ...
    it('resets the count to 1 after expiry (not 0)', async () => {
  ...
  describe('edge cases', () => {
    it('limit of 1 blocks on the second call', async () => {
      const key = 'ip:limit-one'
      expect((await rateLimit(key, 1, 60_000)).ok).toBe(true)
      expect((await rateLimit(key, 1, 60_000)).ok).toBe(false)
    })

    it('a very large limit never blocks within a single test', async () => {
      const key = 'ip:huge-limit'
      for (let i = 0; i < 100; i++) {
        expect((await rateLimit(key, 10_000, 60_000)).ok).toBe(true)
      }
    })
  })
})
