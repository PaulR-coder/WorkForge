  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <h1 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)' }}>{t('invoicesTitle')}</h1>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
          {TABS.map(tabKey => (
            <button key={tabKey} onClick={() => setTab(tabKey)}
              style={{ padding: '5px 12px', borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: 'pointer', border: '1px solid var(--border)', background: tab === tabKey ? 'var(--amber)' : 'var(--bg3)', color: tab === tabKey ? '#080c1a' : 'var(--text3)' }}>
              {t(tabKey)}
