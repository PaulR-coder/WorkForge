...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      color: s.color, background: s.bg, textTransform: 'capitalize', letterSpacing: '.3px',
    }}>
      {s.label}
    </span>
  )
...
export default function EstimatesClient({
  ...
  const [estimates, setEstimates] = useState<Estimate[]>(
    initialEstimates.map(e => ({ ...e, lineItems: (e.lineItems as LineItem[]) ?? [] }))
  )
  ...
  function openCreate() {
    setEditTarget(null)
    setClient(''); setJobType(''); setDescription('')
  ...
  function openEdit(est: Estimate) {
    setEditTarget(est)
    setClient(est.client); setJobType(est.jobType); setDescription(est.description)
  ...
  function updateItem(id: string, field: keyof LineItem, raw: string) {
    setLineItems(prev => prev.map(item => {
      ...
      if (field === 'hours') updated.total = parseFloat(raw) * item.unitPrice || 0
      if (field === 'qty') updated.total = parseFloat(raw) * item.unitPrice || 0
      if (field === 'unitPrice') {
  ...
  const save = useCallback(async () => {
    ...
    setSaving(true)
    const body = { client, jobType, description, lineItems, subtotal, notes, status }
    try {
      let res: Response
      if (editTarget) {
        res = await fetch(`/api/estimates/${editTarget.id}`, {
      ...
      const parsed = { ...data, lineItems: (data.lineItems as LineItem[]) ?? [] }
      setEstimates(prev =>
        editTarget
          ? prev.map(e => e.id === editTarget.id ? parsed : e)
          : [parsed, ...prev]
      )
      setModalOpen(false)
  ...
  }, [client, jobType, description, lineItems, subtotal, notes, status, editTarget, toast])
  ...
  async function updateStatus(est: Estimate, newStatus: Estimate['status']) {
    ...
    if (res.ok) {
      const data = await res.json()
      setEstimates(prev => prev.map(e => e.id === est.id ? { ...e, ...data, lineItems: (data.lineItems as LineItem[]) ?? e.lineItems } : e))
    }
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 2 }}>
            AI-powered quotes for your clients
          </div>
        </div>
        <button onClick={openCreate} className="btn btn-primary btn-sm" style={{ marginLeft: 'auto' }}>
          + New Estimate
        </button>
      </div>
            ...
            <div key={est.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  {est.jobType && (
                    <span style={{ fontSize: 10, color: 'var(--text4)', background: 'var(--bg3)', padding: '2px 7px', borderRadius: 10 }}>
                      {est.jobType}
                    </span>
                  )}
                </div>
                ...
                <div style={{ fontSize: 10, color: 'var(--text4)', marginTop: 4 }}>
                  {est.lineItems.length} line item{est.lineItems.length !== 1 ? 's' : ''} · by {est.createdBy.name} · {new Date(est.createdAt).toLocaleDateString()}
                </div>
              </div>

              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--green)', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                ...
                <button onClick={() => openEdit(est)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--text3)', fontSize: 11, cursor: 'pointer' }}>
                  Edit
                </button>
                <button onClick={() => deleteEst(est)} style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--red)', fontSize: 11, cursor: 'pointer' }}>
                  ✕
                </button>
              </div>
            </div>
      ...
      {modalOpen && (
        <div style={{
          ...
          <div style={{
            ...
            width: '100%', maxWidth: 680, padding: 28, position: 'relative',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)' }}>
                {editTarget ? `Edit ${editTarget.number}` : 'New Estimate'}
              </h2>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 5 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: aiLoading ? 'var(--bg3)' : 'linear-gradient(135deg, #f59e0b, #f97316)',
                    border: 'none', color: aiLoading ? 'var(--text4)' : '#080c1a',
                    fontSize: 11, fontWeight: 800, cursor: aiLoading ? 'not-allowed' : 'pointer',
                    opacity: !description.trim() ? .4 : 1,
                  }}
                >
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)}
                placeholder="Describe the job — the AI will generate line items from this"
                rows={3}
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
                ...
                <button onClick={() => setLineItems(prev => [...prev, newItem()])}
                  style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--text3)', fontSize: 11, cursor: 'pointer' }}>
                  + Add
                </button>
              </div>

              <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 80px 80px 32px', gap: 0, background: 'var(--bg3)', padding: '7px 10px', fontSize: 10, fontWeight: 700, color: 'var(--text4)', letterSpacing: '.3px' }}>
                ...
                {lineItems.map((item, idx) => (
                  <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 80px 80px 32px', gap: 0, padding: '6px 10px', borderTop: idx > 0 ? '1px solid var(--border)' : 'none', alignItems: 'center' }}>
                    <input value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)}
                      placeholder="Description" style={{ padding: '5px 6px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--text)', fontSize: 12, width: '100%' }} />
                    <input
                      value={item.hours ?? item.qty ?? ''}
                      onChange={e => updateItem(item.id, item.hours !== undefined ? 'hours' : 'qty', e.target.value)}
                    ...
                    <input value={item.unitPrice || ''} onChange={e => updateItem(item.id, 'unitPrice', e.target.value)}
                    ...
                    <input value={item.total || ''} onChange={e => updateItem(item.id, 'total', e.target.value)}
                      type="number" min="0" placeholder="0"
                ...
                <div style={{ padding: '10px 10px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 8, alignItems: 'center' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 12, marginBottom: 20 }}>
              <div>
                ...
                <select value={status} onChange={e => setStatus(e.target.value as Estimate['status'])}
                  style={{ width: '100%', padding: '9px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--text)', fontSize: 13 }}>
                  <option value="draft">Draft</option>
                  <option value="sent">Sent</option>
                  <option value="approved">Approved</option>
                  <option value="declined">Declined</option>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setModalOpen(false)} style={{ padding: '9px 18px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--text3)', fontSize: 13, cursor: 'pointer' }}>
                Cancel
              </button>
